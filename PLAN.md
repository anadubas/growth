...
Update `lib/growth_web/telemetry.ex` in the `attach_events/0` function:
...
Update `config/config.exs` to enhance existing `logger_json` setup with OpenTelemetry correlation:
